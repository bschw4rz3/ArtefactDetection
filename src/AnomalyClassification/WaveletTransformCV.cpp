...
WaveletTransformCV::WaveletTransformCV(StringSerivce* stringSerivce)
{
    this->stringSerivce = stringSerivce;
}

void WaveletTransformCV::calculate(std::string imagePath)
{
    ...
    cv::Mat image;
    cv::cvtColor(imageRGB, image, cv::COLOR_BGR2GRAY);
    ...
    if (validateLength(pI01, NUM_ROWS) <= 0) 
        return;
    ...
    std::string firstImagePath = this->stringSerivce->split(imagePath, '.')[0];
    cv::imwrite(firstImagePath + "_Level" + this->stringSerivce->intToString(iMaxLevel).c_str() + "_DWT.jpg", imageDWT);
    ...
    // save resultant image
    cv::imwrite(firstImagePath + "_Level" + this->stringSerivce->intToString(iMaxLevel).c_str() + "_IDWT.jpg", imageIDWT);
...
void WaveletTransformCV::waveletTransform2D(unsigned char* arr, unsigned int rowLen, unsigned int colLen, unsigned short level)
{
...
void WaveletTransformCV::invWaveletTransform2D(unsigned char* arr, unsigned int rowLen, unsigned int colLen, unsigned short level)
{
...
void WaveletTransformCV::rearrange2DFromLR(unsigned char* arr, unsigned int rowLen, unsigned int colLen, unsigned int decimatingColLen, unsigned int decimatingRowIndex)
{
    ...
    const int quartLen = decimatingColLen / 4;
    std::vector<unsigned char> indexMask(quartLen); // unsigned char used instead of bool to maintain compatibility with ANSI C
    // initialize to zero
...
void WaveletTransformCV::revertRearrange2DFromLR(unsigned char* arr, unsigned int rowLen, unsigned int colLen, unsigned int runningColLen, unsigned int runningRowIndex)
{
    ...
    std::vector<unsigned char> indexMask(quartLen);
    // initialize to zero
...
void WaveletTransformCV::rearrange2DFromTC(unsigned char* arr, unsigned int rowLen, unsigned int colLen, unsigned int decimatingRowLen, unsigned int decimatingColumnIndex)
{
    ...
    std::vector<unsigned char> indexMask(quartLen); // unsigned char used instead of bool to maintan compatibility with ANSI C
    // initialize to zero
    for (int k = 0; k < quartLen; k++)
    {
        indexMask[k] = 0;
...
void WaveletTransformCV::revertRearrange2DFromTC(unsigned char* arr, unsigned int rowLen, unsigned int colLen, unsigned int runningRowLen, unsigned int runningColumnIndex)
{
...
unsigned char WaveletTransformCV::validateLength(unsigned short* pI, unsigned int length) // unsigned char used instead of bool to maintan compatibility with ANSI C
{
...
unsigned short WaveletTransformCV::inputAndValidation(unsigned short* pI)
{
...
unsigned int WaveletTransformCV::twoExpLevel(unsigned short iLevel)
{
...
void WaveletTransformCV::crtFlatArr(unsigned char* const pUch, const cv::Mat myImage)
{
    int nr = myImage.rows;
    int nc = myImage.cols;
...
// create cv::Mat array
void WaveletTransformCV::crtMatArr(unsigned char* const pUch, cv::Mat& myImage)
{
    int nr = myImage.rows;
    int nc = myImage.cols;
...
void WaveletTransformCV::printArr2D(unsigned char* arr, unsigned int rowLen, unsigned int colLen)
{
