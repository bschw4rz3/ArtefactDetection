...
std::vector<std::complex<double>> MorletWaveletServiceFFT::moreletWavelet(int n, double freq, double dt)
{
    std::vector<std::complex<double>> wavelet(n);
    double sigma = 1.0; // Breite des Gauﬂfilters
    for (int i = 0; i < n; ++i) {
        double t = (i - n / 2) * dt;
...
std::vector<std::complex<double>> MorletWaveletServiceFFT::convolution(const std::vector<double>& signal, const std::vector<std::complex<double>>& wavelet)
{
    int n = signal.size();
...
std::map<double, std::vector<double>> MorletWaveletServiceFFT::computeScalogram(const std::vector<double>& signal, const std::vector<double>& frequencies, double dt)
{
    int n = signal.size();
    ...
    for (int i = 0; i < m; ++i) {
        ...
        std::vector<std::complex<double>> wavelet = this->moreletWavelet(n, freq, dt);
...
std::map<double, std::vector<double>> MorletWaveletServiceFFT::calculate()
{
    ...
    for (int i = 0; i < n; ++i) {
        signal[i] = sin(2 * M_PI * 5 * i * dt) + 0.5 * sin(2 * M_PI * 20 * i * dt);
    }

    // Frequenzen f¸r das Scalogram
