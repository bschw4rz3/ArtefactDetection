#ifndef HOGServiceCV_H
#define HOGServiceCV_H
...
public:
    HOGServiceCV();
    HOGServiceCV(const size_t blocksize,
        const BLOCK_NORM block_norm = BLOCK_NORM::L2hys);
    HOGServiceCV(const size_t blocksize, const size_t cellsize,
        const BLOCK_NORM block_norm = BLOCK_NORM::L2hys);
    HOGServiceCV(const size_t blocksize, const size_t cellsize, const size_t stride,
        const BLOCK_NORM block_norm = BLOCK_NORM::L2hys);
    HOGServiceCV(const size_t blocksize, const size_t cellsize, const size_t stride, const size_t binning = 9,
        const size_t grad_type = GRADIENT_UNSIGNED, const BLOCK_NORM block_norm = BLOCK_NORM::L2hys);
    ~HOGServiceCV();

    // Copy constructor
    HOGServiceCV(const HOGServiceCV& to_copy);

    // assignment operator
    HOGServiceCV& operator=(const HOGServiceCV& to_copy);
